...
public class CollectorsDemo {
  ...
  public static List<User> createData() {
    return List.of(
      new User("Mohan Matta", 29),
      new User("Sudarsan", 32),
      new User("sriram", 33),
      new User("Noor", 27),
      new User("Noor", 20),
      new User("Mohan Matta", 20)
    );
  }
  ...
  public static void main(String[] args) {
    ...
    Map<String, List<User>> byName = users
      .stream()
      .collect(Collectors.groupingBy(User::getName));
    System.out.println(byName);
    // groupingBy (Function<T,R>,Collector)
    Map<String, List<Integer>> byNameAge = users
      .stream()
      .collect(
    ...
    Map<String, Integer> countByName = users
      .stream()
      .collect(
        Collectors.groupingBy(
          User::getName,
    ...
    Map<String, Integer> actorsToLength = actors
      .stream()
      // .collect(Collectors.toMap(name -> name,name -> name.length()));
      .collect(Collectors.toMap(Function.identity(), String::length));
    System.out.println(actorsToLength);

    Map<Character, List<String>> byFirstLetter = actors
      .stream()
    ...
    // collect(Collector)
    // Collector uses Collectors utility class which consists of functions
    // Collectors
    // toList, toSet, toMap

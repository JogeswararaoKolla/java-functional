...
public class mappingDemo {
    ...
    public static void main(String[] args) {
        ...
        // grouping movies by year and get the max rated movie name.
        Map<String, Optional<Movie>> movieByYearRating = movies.stream()
                .collect(Collectors.groupingBy(Movie::getMovieReleaseYear,
                        Collectors.maxBy(Comparator.comparing(Movie::getRating))));
        System.out.println("movieByYearRating = " + movieByYearRating);

        Map<String, Optional<String>> movieNameByYearRating = movieByYearRating
                .entrySet()
                .stream()
                .collect(Collectors.toMap(
                        entry -> entry.getKey(),
                        entry -> entry.getValue().map(e -> e.getMovieName())
                ));
        ...
        Optional<Map.Entry<String, String>> collect = movieByYearRating
                ...
                .collect(Collectors.toMap(
                        entry -> entry.getKey(),
                        entry -> entry.getValue().map(e -> e.getMovieName()).orElse("No Movie")
                )).entrySet()
                .stream()
